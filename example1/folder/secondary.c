...
void secondary()
{
   	...
   	for (i=0;i<20;i++)  
   	{
      		stem_length[i]=0;j_[i]=0;     //l_stem[i]:length of i-th stem 
      		for (j=0;j<20;j++) 
      		{
           			...
           			if (fmod(j,3)==0&&c[j]==1)  
           			{
						...
						if(i>6&&c[3]==1)
						{
							if((s[i+3][j-3]==1&&s[i-3][j+3]==1)||(s[i-3][j+3]==1&&s[i+3][j-3]==0))
                    						...
                    						{
                    							if(stem_length[order]==1)	{ order=order; }
                    							else
                    							{
                    								order++;
                    								stem_length[order]=stem_length[order]+1;
      		...
      		if(N_stem>1&&N_stem<4)
       		...
       		if(pseudoknot==0)
       		{
       			if(N_stem==2)
       			{
       				for(i=1;i<=N_stem;i++)
         			{ 
             				if(i==1)	{j_[i]=stem_i[2][1]-stem_i[1][stem_length[i]];}
             				else		{j_[i]=stem_j[1][stem_length[1]]-stem_j[i][1];}
             				if(j_[i]>=0&&stem_length[i]>2)	{k_l++;}
             				else 				{k_l=k_l;}   
       			...
       			if(N_stem>2)
       			{
         			...
         			{ 
             				if(i==1)		{j_[i]=stem_i[2][1]-stem_i[1][stem_length[i]];}
             				else if(i==N_stem)	{j_[i]=stem_j[1][stem_length[1]]-stem_j[i][1];}
             				else              	{j_[i]=stem_i[i+1][1]-stem_j[i][1];}
             				if(j_[i]>=0&&stem_length[i]>=2) {k_l++;}
             				else 				{k_l=k_l;}   
             				j_l_all=j_l_all+j_[i];   
         			}
